...
class reader_top_block(gr.top_block):
  ...
  def __init__(self):
    ...
    self.freq     = 910e6                # Modulation freqQuency (can be set between 902-920)
   ...
   ######## File sinks for debugging (1 for each block) #########
    ...
    self.file_sink_to_complex     = blocks.file_sink(gr.sizeof_gr_complex*1, "../misc/data/to_complex", False)
    ...
    self.u_sink.set_gain(self.tx_gain, 0) #I think this does nothing, not sure though
